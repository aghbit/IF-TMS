...
class DoubleEliminationStrategy() extends TournamentStrategy {
  ...
  private def goUp(stack: mutable.Stack[Int], game: Game): mutable.Stack[Int] = {
    if (getLevel(game) == 4) stack
      ...
      //we leave the state where going right is obligatory
  ...
  private def potentialQrtFinal(game: Game, tree: EliminationTree): Int = {
    //1st and 3rd QF are winner branches, 2nd and 4th are looser branches
    if (getLevel(game) <= 2) 0
    else {
      if (findQrtFinal(game) == getGame(tree.root, "ll")) 1
      else
      if (findQrtFinal(game) == getGame(tree.root, "lr")) 2
      else
      if (findQrtFinal(game) == getGame(tree.root, "rl")) 3
      else
      if (findQrtFinal(game) == getGame(tree.root, "rr")) 4
      else
        0
    }
  }
  ...
  private def getLevel(game: Game): Int = {
    // Final has 1st level, Semis have 2nd level and so on
