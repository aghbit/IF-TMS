...
class  DoubleEliminationStrategyTest extends FunSuite with BeforeAndAfter with MockitoSugar {
  ...
  test("generatetreeTest"){
    ...
    //when
    println(tree.root.left.get.left.get.right.get.left.get)
  ...
  test("UpdateTree: Updating first-match in 8-team tournament"){
    ...
    assert(newGame.value.get.guest===winner2,"UpdateTree: Updating first-match in 8-team tournament2")    //blad!!!
    assert(newGameLosers.value.get.host===loser1,"UpdateTree: Updating first-match in 8-team tournament3")
    assert(newGameLosers.value.get.guest===loser2,"UpdateTree: Updating first-match in 8-team tournament4") //blad!!
  }
  //
  ...
  test("UpdateTree: playing 5-team tournament till the final Semis Phase"){
    ...
    game2.value.get.scoreGuest=score2



    //val winnerList = games.map(game=>game.value.winningTeam)
    val loserList = game2.value.get.losingTeam
